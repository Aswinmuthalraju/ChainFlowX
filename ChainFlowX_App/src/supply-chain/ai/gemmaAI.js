...
Description: ${description}`;
  ...
  try {
    console.debug(`[ChainFlowX Gemma] Calling ${model} at ${apiUrl}`);
    ...
    console.debug(`[ChainFlowX Gemma] ✅ classified as "${parsed.eventType}" (confidence ${parsed.confidence})`);
    ...
    console.warn('[ChainFlowX Gemma] ❌ LLM call failed:', e.message, '— using keyword fallback. Check VITE_GEMMA_URL in .env.');
    llmMemory.push('user', `classify (fallback): ${headline}`, 'gemma');
