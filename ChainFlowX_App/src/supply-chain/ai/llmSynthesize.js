...
export async function synthesizeStrategicInsight(eventState) {
  const fallback = templateSynthesisFallback(eventState);
  const cfg = getLLMConfig();
  if (!cfg.baseUrl || !cfg.synthesizeModel) {
    llmMemory.push('user', `strategic (fallback): ${eventState?.classified?.eventType || 'unknown'} ripple:${eventState?.rippleScore?.score ?? 0}`, 'synthesize');
    llmMemory.push('assistant', 'template fallback returned because LLM config is missing', 'synthesize');
    return fallback;
  }
...
${allAlts.length > 0
  ? allAlts.map((a, i) => `  ${i + 1}. ${a.recommended} - +${a.delayDays} days, +$${a.costDelta}/container${a.congestionNote ? ' WARNING ' + a.congestionNote : ''}`).join('\n')
...
Total affected routes: ${affectedRoutes?.length || 0}`;
  ...
  if (!response.ok || !response.data) {
    llmMemory.push('user', `strategic (fallback): ${classified?.eventType || 'unknown'} ripple:${rippleScore?.score ?? 0}`, 'synthesize');
    llmMemory.push('assistant', `template fallback returned: ${response.reason || 'unknown'}`, 'synthesize');
    return fallback;
  }

  const result = { ...response.data, _source: 'llm_synthesize' };
  llmMemory.push('user', `strategic: ${classified?.eventType || 'unknown'} ripple:${rippleScore?.score ?? 0}`, 'synthesize');
  llmMemory.push('assistant', result?.strategicAnalysis?.slice(0, 200) || 'synthesis complete', 'synthesize');
  return result;
}