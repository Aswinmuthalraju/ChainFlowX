...
Based on this data, provide the strategic decision briefing. Be specific. Use the numbers above.`;
  ...
  try {
    console.debug(`[ChainFlowX Qwen] Calling ${model} at ${apiUrl}`);
    ...
    console.debug(`[ChainFlowX Qwen] ✅ Strategic insight generated (urgency: ${result.urgency})`);
    ...
    console.warn('[ChainFlowX Qwen] ❌ LLM call failed:', e.message, '— using template fallback. Check VITE_QWEN_URL in .env.');
    llmMemory.push('user', `strategic (fallback): ${classified?.eventType || 'unknown'} ripple:${rippleScore?.score ?? 0}`, 'qwen');
