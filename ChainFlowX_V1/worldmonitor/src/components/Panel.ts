...
export class Panel {
  ...
  constructor(options: PanelOptions) {
    ...
    if (options.premium && !getSecretState('CHAINFLOWX_API_KEY').present) {
  ...
  public showLocked(features: string[] = []): void {
    ...
    if (isDesktopRuntime()) {
      ctaBtn.addEventListener('click', () => void invokeTauri<void>('open_url', { url: 'https://chainflowx.app/pro' }).catch(() => window.open('https://chainflowx.app/pro', '_blank')));
    } else {
      ctaBtn.addEventListener('click', () => {
        import('@/services/checkout').then(m => import('@/config/products').then(p => m.startCheckout(p.DEFAULT_UPGRADE_PRODUCT))).catch(() => {
          window.open('https://chainflowx.app/pro', '_blank');
        });
      });
    }
