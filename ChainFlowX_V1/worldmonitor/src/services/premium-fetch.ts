 ...
 *
 * Injects a Clerk Bearer token (or CHAINFLOWX_API_KEY as fallback) directly
...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://chainflowx.app').pathname;
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-ChainFlowX-Key')) {
    const res = await globalThis.fetch(input, init);
    reportServerError(res, input);
    return res;
  }

  // 1. CHAINFLOWX_API_KEY from env (desktop / test environments).
  try {
    ...
    if (wmKey) {
      existing.set('X-ChainFlowX-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-ChainFlowX-Key', testerKey);
  ...
  //    none of the tester keys are in CHAINFLOWX_VALID_KEYS).
  try {
