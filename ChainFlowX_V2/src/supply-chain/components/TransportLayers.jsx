...
export default function TransportLayers({ layerVisibility, onVesselsChange, onAircraftChange }) {
  ...
  useEffect(() => {
    ...
    console.log('[ChainFlowX] AIS key present:', !!key);
    if (!key) {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        console.log('[ChainFlowX] AIS WebSocket state:', ws.readyState);
        console.log('[ChainFlowX] Ship message received:', message);
        const pr =
        ...
        console.log('[ChainFlowX] Ship points array length:', shipPoints.length);
      } catch {
        /* ignore parse errors */
      }
    };
  ...
  useEffect(() => {
    async function poll() {
      ...
      try {
        ...
        console.log('[ChainFlowX] OpenSky fetch URL:', url);
        const res = await fetch(url, { headers: { Accept: 'application/json' } });
        console.log('[ChainFlowX] OpenSky response status:', res.status);
        if (res.status === 429) {
        ...
        console.log('[ChainFlowX] Aircraft raw states count:', data.states?.length);
        ...
        console.log('[ChainFlowX] Aircraft filtered count:', filteredAircraft.length);
