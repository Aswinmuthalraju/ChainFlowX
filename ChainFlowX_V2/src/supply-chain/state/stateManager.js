...
export async function runPipeline(event, graph) {
  ...
  // Step 1: keyword (instant) then Layer 3 LLM — keep higher-confidence result
  const kw = keywordClassify(event.headline, event.description || '');
  let classified = validateAndNormalizeClassification(
    keywordToRawClassification(kw),
    event.lat,
    event.lng,
  );
  ...
  const llmClassified = validateAndNormalizeClassification(rawClassified, event.lat, event.lng);
  if (llmClassified.confidence > classified.confidence) {
    classified = llmClassified;
    state.classified = classified;
  }

  // Gate: disruption pipeline for anything with plausible supply-chain relevance.
  // 0.35 passes maritime articles detected by type inference (even if no keyword matched).
  // The live-news auto-trigger feed already filters at 0.6 independently.
  if (classified.supplyChainRelevance < 0.35) {
    return state;
  }
